...
void handleRoot() {
  ...
  html += F("<title>ASI585MC Cooler</title><style>body{margin:0;font-family:Arial,sans-serif;background:#08111f;color:#e8f0ff}"
            ...
            ".grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(260px,1fr));gap:16px}.card{background:#111d32;border:1px solid #243757;border-radius:16px;padding:16px;box-shadow:0 10px 30px rgba(0,0,0,.25)}.wide{grid-column:1/-1}"
            ...
            ".metric{font-size:28px;font-weight:bold}.meta{color:#9fb5d8;font-size:13px}.mini{font-size:12px;line-height:1.5;color:#b7c8e6}.row{display:grid;grid-template-columns:1fr 1fr;gap:10px}.diag{display:grid;grid-template-columns:1fr 1fr;gap:6px 12px}.diag div:nth-child(odd){color:#9fb5d8}.diag div:nth-child(even){text-align:right}.charts{display:grid;grid-template-columns:1fr 1fr 1fr;gap:12px}.chartBox{background:#091221;border:1px solid #243757;border-radius:12px;padding:10px}.chartTitle{font-size:12px;color:#9fb5d8;margin-bottom:8px}.chartBox canvas{width:100%;height:150px;display:block}"
            "@media(max-width:640px){.row{grid-template-columns:1fr}.hero{flex-direction:column;align-items:flex-start}}</style></head><body>");
  html += F("<div class='wrap'><div class='hero'><div><h1>ZWO ASI Smart Cooler</h1><div class='meta'>Firmware <span id='fwVersion'>--</span> | <a id='projectLink' href='#' target='_blank' style='color:#8fc7ff'>GitHub</a></div></div><div><button onclick='refreshStatus()'>Aggiorna</button><button class='alt' onclick='applyBaseTune()'>Base tune PID</button></div></div>");
  ...
  html += F("<div class='card wide'><h2>Trends</h2><div class='charts'><div class='chartBox'><div class='chartTitle'>Cold side and hot side °C</div><canvas id='tempChart' width='320' height='150'></canvas></div><div class='chartBox'><div class='chartTitle'>Ambient and dew point °C</div><canvas id='dewChart' width='320' height='150'></canvas></div><div class='chartBox'><div class='chartTitle'>Peltier power %</div><canvas id='powerChart' width='320' height='150'></canvas></div></div></div>");
  html += F("<div class='card'><h2>Controllo</h2><div class='row'><div><label>Target freddo °C</label><div class='uf'><input id='target_c' type='number' step='0.1'><span>°C</span></div></div><div><label>Margine dew point °C</label><div class='uf'><input id='dew_margin_c' type='number' step='0.1'><span>°C</span></div></div></div><div class='row'><div><label>Duty max 0-1</label><div class='uf'><input id='max_duty' type='number' step='0.01' min='0' max='1'><span>x</span></div></div><div><label>Temperatura max lato caldo °C</label><div class='uf'><input id='max_hot_side_c' type='number' step='0.1'><span>°C</span></div></div></div><div class='row'><div><label>Controllo</label><select id='enabled'><option value='true'>Abilitato</option><option value='false'>Disabilitato</option></select></div><div><label>Sensore caldo</label><select id='hot_sensor_enabled'><option value='true'>Abilitato</option><option value='false'>Disabilitato</option></select></div></div><button onclick='saveConfig()'>Salva controllo</button><button class='warn' onclick='disableCooler()'>Stop</button></div>");
  html += F("<div class='card'><h2>PID</h2><div class='row'><div><label>Kp</label><input id='kp' type='number' step='0.01'></div><div><label>Ki</label><input id='ki' type='number' step='0.01'></div></div><div class='row'><div><label>Kd</label><input id='kd' type='number' step='0.01'></div><div><label>Profilo rapido</label><select id='pid_profile'><option value='manual'>Manuale</option><option value='soft'>Soft</option><option value='normal'>Normal</option><option value='aggressive'>Aggressive</option></select></div></div><button onclick='savePid()'>Salva PID</button><button class='alt' onclick='savePidProfile()'>Applica profilo</button></div>");
  html += F("<div class='card'><h2>Calibrazioni</h2><div class='row'><div><label>Offset freddo °C</label><div class='uf'><input id='cold_offset_c' type='number' step='0.1'><span>°C</span></div></div><div><label>Offset caldo °C</label><div class='uf'><input id='hot_offset_c' type='number' step='0.1'><span>°C</span></div></div></div><div class='row'><div><label>Offset ambiente °C</label><div class='uf'><input id='ambient_offset_c' type='number' step='0.1'><span>°C</span></div></div><div><label>Offset umidita %</label><div class='uf'><input id='humidity_offset_pct' type='number' step='0.1'><span>%</span></div></div></div><div class='row'><div><label>Offset corrente A</label><div class='uf'><input id='current_offset_a' type='number' step='0.01'><span>A</span></div></div><div><label>Scala corrente</label><div class='uf'><input id='current_scale' type='number' step='0.01'><span>x</span></div></div></div><button onclick='saveCalibration()'>Salva calibrazioni</button></div>");
  html += F("<div class='card'><h2>Preset</h2><label>Preset</label><select id='preset_name'><option value='estate'>Estate</option><option value='inverno'>Inverno</option><option value='deep_cooling'>Deep cooling</option></select><button onclick='applyPreset()'>Applica preset</button><button class='alt' onclick='savePreset()'>Salva preset corrente</button><div class='meta' id='presetInfo'>Snapshot di target, dew margin, duty max e protezione lato caldo.</div></div>");
  html += F("<div class='card'><h2>Board Details</h2><div class='mini diag'><div>WiFi signal</div><div id='wifiRssi'>--</div><div>BSSID</div><div id='wifiBssid'>--</div><div>IP</div><div id='boardIp'>--</div><div>BLE</div><div id='bleState'>--</div><div>Free RAM</div><div id='freeHeap'>--</div><div>CPU temp</div><div id='cpuTemp'>--</div></div></div>");
  html += F("<script>function fmt(v,u=''){return Number.isFinite(v)?v.toFixed(2)+u:'--'}function boolVal(id){return document.getElementById(id).value==='true'}function numVal(id){return parseFloat(document.getElementById(id).value)}"
            "const hist={cold:[],hot:[],ambient:[],dew:[],power:[]};function pushHist(k,v){if(!Number.isFinite(v))return;hist[k].push(v);if(hist[k].length>90)hist[k].shift()}function fillField(id,v){if(Number.isFinite(v))document.getElementById(id).value=v}function fillBool(id,v){document.getElementById(id).value=v?'true':'false'}"
            ...
            "function drawChart(id,series,minV,maxV,colors){const c=document.getElementById(id);const x=c.getContext('2d');x.clearRect(0,0,c.width,c.height);x.strokeStyle='#223754';x.lineWidth=1;for(let i=0;i<4;i++){const y=10+i*((c.height-20)/3);x.beginPath();x.moveTo(0,y);x.lineTo(c.width,y);x.stroke()}series.forEach((arr,idx)=>{if(arr.length<2)return;x.beginPath();x.lineWidth=2;x.strokeStyle=colors[idx];arr.forEach((v,i)=>{const px=(i/(Math.max(arr.length-1,1)))*(c.width-10)+5;const py=c.height-10-((v-minV)/Math.max(maxV-minV,0.001))*(c.height-20);if(i===0)x.moveTo(px,py);else x.lineTo(px,py)});x.stroke()})}function redrawCharts(){const temps=[...hist.cold,...hist.hot].filter(Number.isFinite);const dew=[...hist.ambient,...hist.dew].filter(Number.isFinite);const pwr=[...hist.power].filter(Number.isFinite);const tMin=temps.length?Math.min(...temps)-1:0;const tMax=temps.length?Math.max(...temps)+1:10;const dMin=dew.length?Math.min(...dew)-1:0;const dMax=dew.length?Math.max(...dew)+1:10;drawChart('tempChart',[hist.cold,hist.hot],tMin,tMax,['#55d6ff','#ff8f6b']);drawChart('dewChart',[hist.ambient,hist.dew],dMin,dMax,['#6ee7b7','#facc15']);drawChart('powerChart',[hist.power],0,100,['#3a86ff'])}function syncUi(j){coldMetric.textContent=fmt(j.sensor.cold_c,' °C');dewMetric.textContent=fmt(j.sensor.dew_point_c,' °C');currentMetric.textContent=fmt(j.sensor.current_a,' A');hotMetric.textContent=fmt(j.sensor.hot_c,' °C');ambientMetric.textContent=fmt(j.sensor.ambient_c,' °C');dutyMetric.textContent=fmt(j.control.pwm_duty*100,' %');fwVersion.textContent=j.board.firmware_version||'--';projectLink.href=j.board.project_url||'#';boardIp.textContent=j.network.ip||'--';wifiRssi.textContent=Number.isFinite(j.board.wifi_rssi_dbm)?j.board.wifi_rssi_dbm+' dBm':'--';wifiBssid.textContent=j.board.wifi_bssid||'--';bleState.textContent=j.board.ble_connected?('Connected '+j.board.ble_clients):'Idle';freeHeap.textContent=Number.isFinite(j.board.free_heap_bytes)?j.board.free_heap_bytes+' B':'--';cpuTemp.textContent=fmt(j.board.cpu_temp_c,' °C');pushHist('cold',j.sensor.cold_c);pushHist('hot',j.sensor.hot_c);pushHist('ambient',j.sensor.ambient_c);pushHist('dew',j.sensor.dew_point_c);pushHist('power',j.control.pwm_duty*100);redrawCharts();"
